...
;;;
;;; If all files are found, each capy:include* procedure
...
;;; capy:included-forms? in the common case.
;;;
...
(define (capy:include filenames)
  (cons 'begin
        (map capy:include-file filenames)))

(define (capy:include-ci filenames)
  (cons 'begin
        (map capy:include-ci-file filenames)))

(define (capy:include-library-declarations filenames)
  (cons 'begin
        (map capy:include-library-declarations-file filenames)))
...
(define (capy:included-forms-as-list x)
  (if (capy:included-forms? x)
      (cddr (cdddr x))
      #f))

;;; Returns two values.
;;;
;;; Given a begin form that satisfies capy:included-forms?,
;;; returns the <dirname> and <basename>.
;;;
;;; Otherwise returns #f and #f.

(define (capy:source-path-of-included-forms x)
  (if (capy:included-forms? x)
      (let ((y (cdddr x)))
        (values (car y) (cadr y)))
      (values #f #f)))
...
;;; the capy:include procedures.

(define *inclusion-token* (string-copy "included from "))

(define (capy:make-included-begin-form forms dirname basename)
...
(define (capy:include-ci-file filename)
  (parameterize ((case-sensitive? #f))
   (capy:include-file filename)))

(define (capy:include-library-declarations-file filename)
  (capy:include-file filename))
...
(define (capy:include-file filename)
         ...
         (fname
          (call-with-current-continuation
           (lambda (return)
             ...
             #f))))
    (if fname
        (let* ((dirname (capy:directory-of fname))
               (basename (capy:file-name-only fname))
               (forms (capy:include-file-forms fname)))
          (capy:make-included-begin-form forms dirname basename))
        (assertion-violation 'include "file not found" filename))))

(define (capy:include-file-forms filename)
