...
(letrec-syntax 
    ...
    (let* (
        ...
        (define (make-free-name symbol)
            (display (format "~a is a free symbol~%" symbol))
        ...
        ;; so missing bindings must be special-cased below. 
        ...
        (define (syntax-violation who message form . maybe-subform)
            ...
            (subform (cond ((null? maybe-subform) #f)
                ...
                (let* ((form (syntax-debug form))
                       ...
                       (c (if who (condition c1 c2 c3) (condition c2 c3))))
                    (raise c))))
        ...
        (define (expand-cond-expand body-type type form)
            ...
            (error 'cond-expand "NOT YET IMPLEMENTED" body-type type form)
            (let ((form2 (cons (car form)
                                (map (lambda (clause)
                                        (cons (syntax->datum (car clause))
                                            (cdr clause)))
                                    (cdr form)))))
                (capy:cond-expand form2)))
      ...
      (let ((common-env *usage-env*))
        ...
        (define (loop ws                                               ; [R7RS]
          ...
          (cond
           ...
           (else
            (fluid-let ((*usage-env* (car (car ws))))
              (call-with-values
                    ...
                    (case type
                      ...
                      ((include-library-declarations)
                       (match form
                        ((keyword filenames ___)
                         (let* ((decls-as-begin
                                 (capy:include-library-declarations
                                  filenames))
                      ...
                      ((begin)
                       ...
                       (let ()
                         ...
                         (define (extend-search-path)
                           (call-with-values
                            (lambda ()
                              (capy:source-path-of-included-forms form))
                            (lambda (dirname basename)
                              (let* ((paths (current-require-path))
                                     (paths (if (member dirname paths)
                                                ...
                                                (cons dirname paths)))
                                     (begin-forms
                                      (capy:included-forms-as-list form))
                                     (form
        ...
        (call-with-values                                              ; [R7RS]
         (lambda ()                                                    ; [R7RS]
           (loop (map (lambda (e) (cons common-env e))
                      body-forms)
            ...
            (and (not (eq? body-type 'toplevel))
                (not (and (eq? type 'import)                                  ; [R7RS]
                            (or (and (eq? body-type 'program)                 ; [R7RS]
                                    (not (eq? (capy:execution-mode)        ; [R7RS]
                                                'r6rs)))                      ; [R7RS]
                ...
                (let ((exports (scan-exports sets library-type)))           ; [R7RS]
                    ...
                    (lambda (imported-libraries imports)
                        ...
                        (if (or (eq? library-type 'define-library)          ; [R7RS]
                                (and (not (eq? (capy:execution-mode)     ; [R7RS]
                                                'r6rs))                      ; [R7RS]
        ...
        ;;                 ((<local name> . <binding>) ...)
        ;; where <level> ::= <integer>
        ;; FIXME: For primitives, (capy PRIMITIVES) is the library name.
        ...
        ;; Modified for Capy, mainly so Capy's repl will print
        ;; the results.
        (define (repl exps)
            (define (eval1 exp)
