...
;;;
;;;
;;; NOTE(Adel Prokurov): Updated for Capy Scheme. Uses core-hashtable and tuples.
;;;
;;;

(define (print-with-shared-structure obj outport write)
    (define (lookup key state)
        (core-hashtable-ref state key #f))
    (define (present? key state)
        (core-hashtable-contains? state key))

    (define (updated-state key val state)
        (core-hashtable-set! state key val)
        state)
    ...
    ; make-core-hashtable defaults to `eq?`
    (let* ((state (make-core-hashtable))
