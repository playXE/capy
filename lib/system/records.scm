...
(define (record-type-descriptor? obj)
    (if (tuple? obj)
        ...
        #f))
(define (rtd-name rtd) (tuple-ref rtd 1))
(define (rtd-parent rtd) (tuple-ref rtd 2))
(define (rtd-uid rtd) (tuple-ref rtd 3))
(define (rtd-sealed? rtd) (tuple-ref rtd 4))
(define (rtd-opaque? rtd) (tuple-ref rtd 5))
(define (rtd-fields rtd) (tuple-ref rtd 6))
...
(define (rtd-total-field-count rtd)
    (+ (length (rtd-fields rtd)) (rtd-inherited-field-count rtd)))
    ...
    (let ((opaque? (or opaque? (and parent (rtd-opaque? parent))))
          (fields
            (map (lambda (field)
                    (if (pair? field)
                        (cond 
                            [(eq? (car field) 'mutable) (cons #t (cadr field))]
                            [(eq? (car field) 'immutable) (cons #f (cadr field))]
                            [else (assertion-violation 'make-record-type-descriptor "malformed field specifiers" fields)]
                        )
            ...
            [(core-hashtable-ref nongenerative-record-types uid #f) => 
              (lambda (current)
                (if (and (eqv? uid (rtd-uid current))
                  ...
                  (assertion-violation
                    'make-record-type-descriptor
                    "mismatched subsequent call for nongenerative record-type" (list name parent uid sealed? opaque? fields))))]
            [else 
        ...
        (if parent 
            (let ([parent-field-count (rtd-total-field-count parent)])
                (lambda (p)
                    (lambda field-values 
                        (call-with-values 
                            (lambda () (split-at field-values parent-field-count))
...
(define (make-record-constructor-descriptor rtd parent protocol)
    ...
    (let (
        [custom-protocol? (and protocol #t)]
        [protocol (or protocol (default-protocol rtd))]
        [parent 
            (or parent
                (cond 
                    [(rtd-parent rtd) => (lambda (rtd) (make-record-constructor-descriptor rtd #f #f))]
                    [else #f]))])
