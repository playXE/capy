...
use crate::compile::r7rs_to_value;
use crate::raise_exn;
...
impl Port {
    pub fn new(thread: &mut Thread) -> Handle<Self> {
        thread.allocate(Self {
            header: ObjectHeader::new(Type::Port),
            lock: RawMutex::new(),
            handlers: Value::encode_null_value(),
            bytes: Value::encode_null_value(),
            lookahead: [0; 8],
            buf: null_mut(),
            buf_head: null_mut(),
            buf_tail: null_mut(),
            name: Value::encode_null_value(),
            transcoder: Value::encode_null_value(),
            mark: 0,
            fd: -1,
            lookahead_size: 0,
            buf_size: 0,
            ...
            opened: false,
            parsed: Value::encode_undefined_value(),
        })
    }
}
        ...
        loop {
            ...
            if n < 0 {
                if errno::errno().0 == libc::EINTR {
                    ...
                    return raise_exn!(
                        FailFilesystemErrno,
                        &[Value::encode_int32(errno::errno().0)],
                        "port read failed"
                    );
                }
            }
    ...
    while rest > 0 {
        unsafe {
            ...
            let result = apply(
                ...
                &[bv, Value::encode_int32(0), Value::encode_int32(rest as i32)],
            )?;
            if result.is_int32() {
                let written = result.get_int32() as usize;
...
fn device_set_mark(mut port: Handle<Port>, mark: i64) -> Result<(), Value> {
    if port.typ == SCM_PORT_TYPE_NAMED_FILE {
        ...
        apply(
            ...
            &[Value::encode_int32(mark as i32)],
        )?;
    }
...
fn device_close(mut port: Handle<Port>) -> Result<(), Value> {
    if port.typ == SCM_PORT_TYPE_NAMED_FILE {
        ...
        if clos.is_procedure() {
            apply(vect.vector_ref(SCM_PORT_HANDLER_CLOSE), &[])?;
        }
    }
...
fn init_port_transcoder(mut port: Handle<Port>) {
    ...
    if transcoder.is_bytevector() {
        port.codec = transcoder.bytevector_ref(0);
        port.eol_style = transcoder.bytevector_ref(1);
        port.error_handling_mode = transcoder.bytevector_ref(2);
...
pub fn port_open_std(
    ...
    port.handlers = false.into();
    port.bytes = false.into();
...
pub fn port_open_file(
    ...
    port.handlers = Value::encode_bool_value(false);
    port.bytes = Value::encode_bool_value(false);
    port.name = name;
    port.direction = direction;
    port.file_options = file_options;
    port.buffer_mode = buffer_mode;
...
pub fn port_make_custom_port(
    ...
    port.bytes = make_bytevector(Thread::current(), SCM_PORT_CUSTOM_BUFFER_SIZE as _).into();
    port.name = name;
    port.direction = direction;
    port.transcoder = transcoder;
    port.buffer_mode = SCM_PORT_BUFFER_MODE_NONE;
...
pub fn port_make_transcoder_port(
    ...
    textual.codec = transcoder.bytevector_ref(0);
    textual.eol_style = transcoder.bytevector_ref(1);
    textual.error_handling_mode = transcoder.bytevector_ref(2);
...
pub fn port_has_port_position_pred(port: Handle<Port>) -> bool {
    match port.typ {
        ...
        SCM_PORT_TYPE_CUSTOM => {
            ...
            vect.vector_ref(SCM_PORT_HANDLER_GET_POS).is_procedure()
        }
    ...
    if port.opened {
        match port.typ {
            ...
            SCM_PORT_TYPE_BYTEVECTOR => {
                return port.bytes.bytevector_len() as i32 - port.mark as i32;
            }
...
pub fn port_eof(port: Handle<Port>) -> Result<bool, Value> {
    if port.opened {
        match port.typ {
            ...
            SCM_PORT_TYPE_BYTEVECTOR => {
                return Ok((port.bytes.bytevector_len() as i64) <= port.mark);
            }
...
pub fn port_lookahead_byte(mut port: Handle<Port>) -> Result<i32, Value> {
    if port.opened {
        match port.typ {
            SCM_PORT_TYPE_BYTEVECTOR => {
                if port.bytes.bytevector_len() <= port.mark as usize {
                    return Ok(libc::EOF);
                }

                return Ok(port.bytes.bytevector_ref(port.mark as usize) as i32);
            }
    ...
    if port.opened {
        match port.typ {
            SCM_PORT_TYPE_BYTEVECTOR => {
                ...
                let c = port.bytes.bytevector_ref(port.mark as usize) as i32;
    ...
    if port.opened {
        match port.typ {
            SCM_PORT_TYPE_BYTEVECTOR => {
                ...
                let available = port.bytes.bytevector_len() - port.mark as usize;
                let len = p.len().min(available);

                for i in 0..len {
                    p[i] = port.bytes.bytevector_ref(port.mark as usize + i) as u8;
                }
    ...
    if port.opened {
        'top: loop {
            ...
            if b > 127 {
                ...
                return Ok(Value::encode_char(char::from_u32(ucs4).unwrap()));
            }

            return Ok(Value::encode_char(b as u8 as char));
        }
    }
...
pub fn port_get_bytevector(port: Handle<Port>) -> Value {
    ...
    if port.buf.is_null() {
        make_bytevector(Thread::current(), 0).into()
    } else {
...
pub fn port_get_string(port: Handle<Port>) -> Result<Value, Value> {
    if port.buf.is_null() {
        Ok(make_string(Thread::current(), "").into())
    } else {
        ...
        let s = make_string(Thread::current(), s);
    ...
    match port.codec {
        ...
        SCM_PORT_CODEC_LATIN1 => port_lookahead_byte(port).map(|x| {
            if x == -1 {
                ...
                Value::encode_char(x as u8 as _)
            }
        }),
        _ => todo!("UTF-16 codec"),
    }
...
pub fn port_has_set_port_position_pred(port: Handle<Port>) -> bool {
    match port.typ {
        ...
        SCM_PORT_TYPE_CUSTOM => {
            ...
            vect.vector_ref(SCM_PORT_HANDLER_SET_POS).is_procedure()
        }
...
pub fn port_puts(port: Handle<Port>, s: &str) -> Result<(), Value> {
    ...
    if port.typ == SCM_PORT_TYPE_CUSTOM {
        if port.handlers.vector_ref(SCM_PORT_HANDLER_TEXTUAL).is_true() {
            port_flush_output(port)?;
        }
    }
    Ok(())
...
pub fn port_read(mut port: Handle<Port>) -> Result<Value, Value> {
    if port.parsed.is_undefined() {
        ...
        let mut toplevel = Value::encode_null_value();
